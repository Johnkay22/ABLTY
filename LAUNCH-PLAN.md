...
**Status synced:** 2026-09-11, task 0.1 written and opened as [#125](https://github.com/Johnkay22/ABLTY/pull/125) (not merged, migrations not applied).
...
- 2026-09-11: **Do before beta.** Client roles (`anon`, `authenticated`) hold more than SELECT on the `public_usernames` view via Supabase's default table grants; they only ever need SELECT. Restrict the view to SELECT-only for those roles (`REVOKE INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON public.public_usernames FROM anon, authenticated;`). One statement, independent of the 0.1 rollout order, nothing in `app.html` or the Worker writes through the view. Full context was given with the 0.1 report; deliberately not folded into #125 because 0.1(d) is tracking-only.
