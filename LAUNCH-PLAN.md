...
**Status synced:** 2026-09-11, task 0.1 written and opened as [#125](https://github.com/Johnkay22/ABLTY/pull/125), revised three times the same day (view grants + consent ownership; per-account consent storage + the sign-in consent gate; user-bound local account cache), then three times more from review (password-recovery prompt queued behind the gate; login honours the gate result, gate resolution bound to the user id, and a new migration `20260913000001` fixing the community RV reveal date; 2026-09-13 revision 6: stale auth completions can no longer approve another account's gate or overwrite the active account's local state; revision 7 same day: the outer auth callers are guarded before their SDK call resolves, and a password reset for the account being entered is kept while another account is entered or gated). Final review of #125 at `5212df6` (2026-09-13) found no remaining merge blockers and considers it safe to merge, subject to the planned physical-device rollout verification. Rollout step 1 is done: `20260911000001` and `20260911000002` are applied live. Not merged; `20260911000003` and `20260913000001` not applied.
**App version on main:** 2026.09.03.1 / sw cache ablty-v70 (#125 carries 2026.09.11.1 / ablty-v71)
**Status:** Phase 0 in progress. 0.1 is in PR #125, reviewed clean at `5212df6` and cleared to merge; the task stays open until the remaining migrations and phone testing are done. Remaining rollout order, in this sequence: (1) merge after the blockers are resolved and reviewed; (2) confirm the new frontend is serving and a refreshed installed PWA runs app 2026.09.11.1 with cache `ablty-v71`; (3) apply `20260913000001` (Chicago reveal policy); (4) apply `20260911000003` (signup trigger) LAST; (5) physical-phone testing of signup, email confirmation, consent, password login and password reset. Done: 0.1b (#122), 0.3 (#123). 0.2 is verification-only and can run any time.
...
|---|---|---|---|---|---|
| 0.1 | Signup completeness, in one PR: (a) `auth.users` trigger that inserts `public.profiles` and `public.user_settings` with `ON CONFLICT DO NOTHING`; (b) change `onSignedIn` so legal-acceptance fields are written onto an existing profile when they are null (today they are written only inside `if (!profile)`, so a trigger-created row would never get consent); (c) a small RPC/function, callable by anon, that answers only whether a username is taken, and wire the signup form to show "That username is already being used. Try another."; (d) commit the live-but-untracked schema into `supabase/migrations` (`is_tester`, `community_rv_targets`, `public_usernames` view, and the three early migrations missing from the folder). No orphan backfill. | Profiles are created client-side today. Interrupted signups leave auth accounts with no profile. The username-taken check currently queries `profiles` as anon against an own-row SELECT policy, so it always returns empty and every name looks free. Collisions only surface later as `seeker_xxxx`. A trigger without the legal-write change silently drops consent recording. The repo migrations folder is not a full record of the live database; `community_rv_targets` and `public_usernames` must be tracked because the Daily Community RV Challenge is coming. | [ ] | [#125](https://github.com/Johnkay22/ABLTY/pull/125) | Written 2026-09-11, tested on a local PostgreSQL 17.5 replica of the relevant live schema (64 SQL assertions, harness local to the author's machine, not committed) plus 91 Node tests (`tests/auth-consent.test.js`, committed; run `node tests/auth-consent.test.js`) running the real `onSignedIn`/`hydrateProfileFromSession`/`reconcileAuthState`/`checkAuthCallback`/`resumeSessionAfterPayment`/`handleSignup`/`handleLogin`/Google callback/consent gate/`renderSettingsState`/`loadAnalyticsFromCloud` extracted from `app.html` against a mock client; `node tests/check-app-syntax.js` compiles every inline script block. Not merged, not device-verified. Live re-check 2026-09-11: still 5 users / 5 profiles / 5 settings / 0 orphans, so no backfill. Revision 2 (same day): `public_usernames` write grants revoked in `20260911000001`; consent removed from localStorage only after a confirmed write. Revision 3 (same day): consent storage split into an unsubmitted draft (`ablty_pending_legal_acceptance`, signup form only) and a per-account record (`ablty_legal_acceptance_pending:<userId>`) that only that user id can consume; plus a **sign-in consent gate** so no account with null `terms_accepted_at`/`privacy_accepted_at` enters the app (needed because Google `signInWithIdToken` creates accounts with no Terms step). Revision 4 (same day): the local username/tier/`username_changed_at` cache is bound to the Supabase user id (`ablty_cache_user_id`); when the profile is temporarily unreadable and the offline verification lets the user in, their own cached state is kept instead of being overwritten with Seeker/free, and another user id never sees it. The database profile still wins whenever it loads. Revision 5 (2026-09-13, Codex P2s): `handleLogin` now honours the boolean from `onSignedIn`, so signing out from the consent gate no longer closes the login screen or shows "Signed in."; `ensureLegalConsent` resolves an open gate only when the completing request is for the same user id, so a late result for another account can no longer dismiss the current account's gate (a new gate for a different user still supersedes the old one); and the `community_rv_targets` SELECT policy tracked in `20260911000001` compared `target_date` with `CURRENT_DATE` (UTC) while reading the 23:11 clock in Chicago, so targets were revealed from 19:00 CDT instead of 23:11. New migration `20260913000001_community_rv_targets_reveal_chicago_date.sql` replaces only that policy with the Chicago calendar date on both comparisons. It is not applied yet; nothing reads that table before the Community RV Challenge, so it goes in rollout step 3. Revision 6 (2026-09-13, merge blockers from review): two stale-completion problems. (a) `submitLegalGate` resolved whatever gate was showing when its update and read-back returned, so account A's delayed save could approve account B's gate, or write its error onto it; it now returns unless `_legalGate` is still the exact gate it was made on (the acceptance stays recorded for A). (b) `completeSignIn`, `hydrateProfileEntry`, `renderSettingsState` and `loadAnalyticsFromCloud` committed `ablty_logged_in`, the account cache, the email flag, `_enteredUserId` or merged cloud rows after awaiting the network with no check that the account was still the one being entered, so a delayed Premium profile for A could overwrite B's Free cache, and a delayed session restore could log A back in after sign-out. Fix: an auth generation counter (`_authGen`) that changes whenever the active account changes (`beginAuthContext`) or signs out (`endAuthContext`, from `clearLocalAuthCache` and `handleSignOut`); every async auth operation captures it first and returns a non-success result before any side effect when it has moved on, so the password and Google login callers cannot announce success for a stale entry. Same-account overlaps (INITIAL_SESSION, SIGNED_IN, token refresh, explicit login) share one generation and cooperate as before. In-flight entries are counted per generation, so a hung stale entry cannot block the password-recovery prompt; the pending recovery is bound to the user id in the PASSWORD_RECOVERY session, cancelled on sign-out or account replacement, and never opens for another account. The guest-data choice and the cloud load are discarded if the account changed while they were open or loading. A genuinely new sign-in for a different user supersedes the previous account's gate (account switch); a late completion never starts a new sign-in, so it cannot. Revision 7 (2026-09-13, second review pass): (a) the outer callers were still unguarded before their own SDK call: `reconcileAuthState`, `checkAuthCallback`, the payment-return re-entry (now `resumeSessionAfterPayment`), `handleLogin`, the Google callback and `handleSignup` awaited `getSession`/`setSession`/sign-in and then handed the result to `onSignedIn`/`hydrateProfileFromSession`/`clearLocalAuthCache`, so a delayed empty session could clear an account that had logged in meanwhile and a delayed result for A could start A's context and replace B's gate. Each now reads `_authGen` before the call and acts on the result only if nothing changed or the result names the already-active account (`isAuthResultUsable`), which keeps the SDK's own SIGNED_IN for the same user cooperating; the settings refresh also ignores a session naming someone other than the active account. (b) `onPasswordRecoveryEvent` now makes the recovery session's account the active one before recording the reset, so a reset for B arriving while A is entered or at its gate is kept until B completes consent and entry, opens exactly once for B and never for A; sign-out, a later unrelated login and an obsolete event still cancel it. **Review outcome (2026-09-13, head `5212df6`):** no remaining merge blockers; safe to merge, subject to the physical-device verification in rollout step 5. 0.1 remains unchecked until steps 2 through 5 are complete. **Rollout status:** step (1) DONE 2026-09-11: `20260911000001` applied live as version `20260911224746` and `20260911000002` as `20260911224850` (Supabase `apply_migration` assigns its own version stamp; the file names stay as they are in the repo). Verified live: anon/authenticated SELECT on `public_usernames` still works, INSERT/UPDATE/DELETE denied (42501), `username_is_taken` true/false correct via SQL and via the REST RPC with the anon key. **Do not edit those two files again; any further database change is a new migration.** Remaining, in this order: (2) merge #125 after the blockers are resolved and reviewed, then confirm the new frontend is serving and a refreshed installed PWA reports app 2026.09.11.1 and cache `ablty-v71`; (3) apply `20260913000001` (Chicago reveal policy; independent of the app, no user-visible effect today); (4) apply `20260911000003` (signup trigger) LAST, because the old `onSignedIn` skips the consent write when a profile already exists; (5) physical-phone testing: signup, email confirmation, consent, password login, password reset. Expect the 4 existing profiles with null consent (including Johnny's and the testers') to see the acceptance screen once on their next sign-in or app open after 2026.09.11.1 ships. Do NOT re-apply the four record-only files (`20260322070144`, `20260406073925`, `20260422061145`, `20260815131016`); their versions are already in the live history. Availability check is exact-match, case-sensitive, matching the live `UNIQUE (username)` constraint (see DISCOVERED ISSUES). Trigger fallback is `seeker_` + first 8 alphanumerics of the user id, identical to the client's `fallbackUsername`. Trigger rollback: `DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;`. `public_usernames` SECURITY DEFINER intent is documented in `20260911000001`. Check `[x]` only after the trigger is live and a real signup on a phone shows profile + settings + consent recorded. |
...
- 2026-09-13: **Premium/tester lock covers UPDATE only.** `enforce_profiles_tier_lock()` is a BEFORE UPDATE trigger, while `profiles` has own-row INSERT (`Users insert own profile`, `20260322070144`) and own-row DELETE (`sec_profiles_delete_own`, `20260519000002`) policies. An authenticated user calling the API directly (not the app) can therefore remove their own profile row and insert a new one with `tier`/`is_tester` values of their choosing; the trigger never sees an INSERT. Pre-existing, not introduced or widened by 0.1 (`20260911000003` only adds rows for brand-new auth users with `tier = 'free'`). **Requires a separate new migration before paid launch** (for example extend the lock to BEFORE INSERT so client roles can only insert `free`/`false`, and reconsider whether the client needs a DELETE policy at all now that account deletion goes through the Worker). Do not fold into #125; do not edit the already-live migrations.
