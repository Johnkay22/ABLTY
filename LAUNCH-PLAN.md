...
**Status synced:** 2026-09-11, task 0.1 written and opened as [#125](https://github.com/Johnkay22/ABLTY/pull/125), revised same day (view grants, consent ownership); not merged, migrations not applied.
**App version on main:** 2026.09.03.1 / sw cache ablty-v70 (#125 carries 2026.09.11.1 / ablty-v71)
**Status:** Phase 0 in progress. 0.1 is in PR #125 awaiting: migrations applied in the order in its Notes, merge, device verification. Done: 0.1b (#122), 0.3 (#123). 0.2 is verification-only and can run any time.
...
- 2026-09-11: `public_usernames` was writable by client roles (Supabase default grants on a plain, auto-updatable view owned by a BYPASSRLS role, so writes reached `profiles` past its own-row RLS). **Fixed in #125**: `20260911000001` revokes INSERT/UPDATE/DELETE/TRUNCATE/REFERENCES/TRIGGER from `anon` and `authenticated`, keeps SELECT. Takes effect live when that migration is applied (step 1 of the 0.1 rollout). A read-only audit of every view in the live database on 2026-09-11 found it was the only view with any `anon`/`authenticated` grants; `extensions.pg_stat_statements*` are not updatable and `vault.decrypted_secrets` is not granted to client roles.
...
**Username availability RPC (after 0.1 lands):** `select public.username_is_taken('name');` exact, case-sensitive, anon-callable.
